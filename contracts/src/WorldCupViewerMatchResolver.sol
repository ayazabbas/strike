...
/// @title WorldCupViewerMatchResolver
/// @notice Resolves USDT parimutuel World Cup markets from Flap's WorldCupViewer read-only contract.
/// @dev Grant this contract ADMIN_ROLE on the target USDTParimutuelV3Factory before calling resolveMarket.
contract WorldCupViewerMatchResolver is AccessControl, ReentrancyGuard {
    ...
    IWorldCupViewer public immutable viewer;
    ...
    function resolveMarket(uint256 marketId) external onlyRole(KEEPER_ROLE) nonReentrant {
        ...
        IWorldCupViewer.MatchViewResult memory result = _readViewerResult(config.viewerMatchId, config.queryType);
    ...
    function _readViewerResult(uint256 viewerMatchId, QueryType queryType)
        internal
        view
        returns (IWorldCupViewer.MatchViewResult memory)
    {
